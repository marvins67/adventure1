package com.iterevg.game.adventure1;

public class Constants {

    public static final String NAME = "Adventure 1";
    public static final int SCR_WIDTH = 320;
    public static final int SCR_HEIGHT = 200;
    public static final int SCALE = 4;
    public static final int GAME_WIDTH = SCR_WIDTH * SCALE;
    public static final int GAME_HEIGHT = SCR_HEIGHT * SCALE;
    public static final int SPR_WIDTH = 44 * SCALE;
    public static final int SPR_HEIGHT = 56 * SCALE;
    public static final int SPR_SIZE = SPR_WIDTH * SPR_HEIGHT;
    public static final double FRAMERATE = 8.0;
    public static final int ONE_SECOND = 1000000000;
}
